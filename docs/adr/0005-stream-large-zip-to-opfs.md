...
- CDN requestは最大3件まで先行開始できるが、ZIP writerは選択順に一件ずつconsumeする
- OPFS writeは最大1 MiBの有界バッファで集約する
...
- 少数の先行requestにより、ファイル間のCDN応答待ちを重ねられる
...
- 最大2件の先行responseと1 MiB write bufferの追加メモリを使用する
