...
実装状況: GSC-01からGSC-27と順序優先収集フローは実装済みです。任意常時表示はChrome実機確認済みです。下方向操作と順序優先フローの個別確認はリリースゲートとして残します。
...
4. active状態のガイドでユーザーが「1画面戻る」または「1画面進む」を押す。
5. メッセージ領域を選択した方向へ表示高の80%だけ移動する。
...
2026-07-17にProject ownerがIssue #22の任意権限の許可・解除、launcher自動表示、popupとの状態同期、二重起動防止をChrome実機で確認し、問題なしと判断しました。ChromeバージョンとOSは共有されていないため推測で補完しません。
