...
- 単体テスト: Vitest + jsdom
- E2E: Puppeteer とローカル fixture ページ
...
ユーザー
  ...
  ▼
Popup UI
  │ scan request / candidate list
  ▼
Content Script ── DOM Adapter ── Discord の現在の DOM
...
service worker は停止・再起動される前提とし、必要な状態は小さなシリアライズ可能データとして `chrome.storage.session` へ置く案を実装時に検証します。メディア本体や完全な URL 履歴を永続保存しません。
...
初期 manifest の候補は以下です。

| 権限 | 用途 | 方針 |
| --- | --- | --- |
| `activeTab` | ユーザーが起動した対象タブへの一時アクセス | 必須候補 |
| `scripting` | 対象タブへスキャン処理を注入 | 必須候補 |
| `downloads` | 保存開始と状態監視 | 必須 |
| `storage` | UI 設定と短命な処理状態 | 必要性を検証 |

`tabs`, `cookies`, `webRequest`, `<all_urls>` は要求しません。`discord.com` の恒久的な host permission よりも `activeTab` を優先し、使い勝手と実現性を spike で検証します。CDN への `fetch()` が必要になった場合だけ、対象を限定した optional host permission を検討します。
...
type ExtensionMessage =
  | { type: 'SCAN_VISIBLE_MEDIA'; requestId: string }
  | { type: 'SCAN_RESULT'; requestId: string; candidates: MediaCandidate[] }
  | { type: 'START_DOWNLOADS'; requestId: string; candidateIds: string[] }
...
- 候補単位の失敗分離
- 最大同時ダウンロード数を 3 程度から検証
- 自動リトライは一時的なネットワークエラーに一回だけ。403/404 はリトライしない
- エラー表示から URL クエリと個人情報を除去
- service worker 再起動後に `chrome.downloads.search()` で既知 ID の状態を再照合
...
- content script と popup 間のメッセージング
