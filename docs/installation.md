...
- バージョン付きの配布 ZIP
- ZIP の SHA-256
- 対象バージョンのCHANGELOG節を本文にしたGitHub Release
...
8. 表示された拡張機能名、バージョン、権限をGitHub Release本文と照合する。
9. 拡張機能をツールバーへ固定する。
...
配布ファイル名を `discord-media-exporter-0.7.0-chrome.zip` とした例です。

### Windows PowerShell

```powershell
Get-FileHash .\discord-media-exporter-0.7.0-chrome.zip -Algorithm SHA256
```

### macOS

```bash
shasum -a 256 discord-media-exporter-0.7.0-chrome.zip
```

### Linux

```bash
sha256sum discord-media-exporter-0.7.0-chrome.zip
```
...
- `.output/discord-media-exporter-0.7.0-chrome.zip`
- `.output/discord-media-exporter-0.7.0-chrome.zip.sha256`

配布物へ影響する変更が`main`へ反映された場合は、GitHub Actionsでも同じ検証と生成を行い、`v<version>` GitHub Releaseへこの2ファイルを添付します。利用者へ渡す前に、Releaseのタグとファイル名が対象バージョンに一致し、workflowが成功していることを確認します。

## 更新

自動更新はありません。

1. 新しいGitHub ReleaseからZIP、SHA-256、CHANGELOG由来の変更内容を確認する。
2. SHA-256 を確認する。
...
更新を中止し、GitHub Release本文の権限差分と配布元を確認してください。本プロジェクトが使用する基本権限は`activeTab`、`scripting`、`downloads`、`storage`、`offscreen`です。

任意権限として、メディアZIP利用時だけDiscord CDN 2ホストへのアクセスを要求し、処理後に解放します。また「Discordで開始ボタンを常に表示」をONにした場合だけ`discord.com`へのサイトアクセスを要求し、OFFにするまで保持します。常時表示を利用しない場合はこのDiscordサイト権限を許可せず、従来の`activeTab`方式を利用できます。恒久的な必須host permissionは使用しません。
