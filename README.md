...
現在の開発バージョンは`0.10.0`です。Chrome Web Storeでは公開せず、GitHub ReleaseのZIPをDeveloper modeで読み込みます。
...
- ChromeのUI言語に追従する日本語・英語UIと、popupからの言語上書き
...
popupの「表示言語」から「自動・日本語・English」を選択できます。「自動」はChromeのUI言語が日本語なら日本語、それ以外の言語では英語を表示します。変更はpopupとDiscord画面内のガイドへ再読み込みなしで反映され、端末ローカルの`chrome.storage.local`だけに保存されます。

Chromeがmanifestから表示する拡張機能名・説明はChromeの言語に従います。popup内の言語設定では変更されません。
