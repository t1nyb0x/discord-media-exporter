...
- 同じチャンネルをスクロールしながら複数回確認できる、最大500件のセッション内候補収集
...
2. 拡張機能のアイコンを押す
3. 現在のメッセージ表示領域内に見えているメディアを確認する
4. 必要に応じてDiscordを自分でスクロールし、拡張機能を再度開いて確認を繰り返す
5. 同じチャンネルで累積された一覧から保存対象を選択する
6. 個別保存またはZIP保存でローカルへ保存する

各確認操作ではその時点で画面内に見えている項目だけを扱います。自動スクロール、Discord の非公開 API の呼び出し、ユーザートークンや Cookie の取得、バックグラウンド巡回は行いません。候補URLは同じチャンネルの収集結果としてChromeセッション中だけ保持し、UIから明示的にクリアできます。
