...
- 利用者向けの機能追加・修正を行う作業ブランチでは、PR作成前に`package.json`を未公開のSemVerへ更新し、同じブランチで`CHANGELOG.md`へ変更を記録する
- 後方互換のある機能追加や新しいUI・操作はminor、後方互換のある不具合・セキュリティ修正はpatch、互換性を壊す操作・データ形式・権限境界の変更はmajorを更新する
- ドキュメント、テスト、CIだけの変更では、配布物や利用者向け動作が変わらない限りバージョンを更新しない
- 複数の変更を一つのリリースへ意図的にまとめる場合は、対象バージョンを一度だけ更新し、コミットごとに重ねて更新しない
